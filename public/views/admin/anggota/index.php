...
<div class="row">
            ...
            <table class="table table-white" id="dataTable">
                ...
                <tbody>
                    ...
                    foreach ($anggota as $key => $a) : ?>
                        <tr>
                            <td><?= $i++ ?></td>
                            <td><?= TextHelper::limitText($a['username'], 20) ?></td>
                            <td><?= $a['notelp'] ?></td>
                            <td><?= $a['level'] ?></td>
                            <td class="d-flex justify-content-center gap-3 align-items-center">
                                <button class="btn btn-info" type="button" data-bs-toggle="offcanvas" data-bs-target="#detail<?= $a['id'] ?>" aria-controls="detail">Detail</button>
                                <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#edit<?= $a['id'] ?>">Edit</button>
                                <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#hapus<?= $a['id'] ?>">Hapus</button>
                            </td>
                        </tr>
                        ...
                        <div class="offcanvas offcanvas-end" tabindex="-1" id="detail<?= $a['id'] ?>" aria-labelledby="detailLabel">
                            ...
                            <div class="offcanvas-body">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" disabled readonly value="<?= $a['nama'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="username" class="form-label">Username</label>
                                    <input type="text" class="form-control" disabled readonly value="<?= $a['username'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email</label>
                                    <input type="text" class="form-control" disabled readonly value="<?= $a['email'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="level" class="form-label">Level</label>
                                    <input type="text" class="form-control" disabled readonly value="<?= $a['level'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="notelp" class="form-label">No telp.</label>
                                    <input type="text" class="form-control" disabled readonly value="<?= $a['notelp'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="notelp" class="form-label">Status</label>
                                    <input type="text" class="form-control" disabled readonly value="<?= $a['status'] == 1 ? 'Aktif' : 'Tidak Aktif' ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="notelp" class="form-label">Alamat</label>
                                    <textarea type="text" class="form-control" disabled readonly><?= $a['alamat'] ?></textarea>
                                </div>
                                <div class="mb-3">
                                    <label for="notelp" class="form-label">Dibuat</label>
                                    <input type="text" class="form-control" disabled readonly value="<?= DateHelper::formatIndonesianDateTime($a['dibuat']) ?>">
                                </div>
                            </div>
                        </div>

                        <!-- Edit Modal -->
                        <div class="modal fade" id="edit<?= $a['id'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editLabel<?= $a['id'] ?>" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form method="post" action="<?= Routes::base('anggota/edit') ?>">
                                        ...
                                        <div class="modal-body">
                                                ...
                                                <div class="input-group">
                                                    <select class="form-select" id="biodata" name="id_biodata">
                                                        ...
                                                        <?php foreach ($biodata as $b) : ?>
                                                            <option value="<?= $b['id_biodata'] ?>" <?= $b['id_biodata'] == $a['id_biodata'] ? 'selected' : '' ?>><?= $b['nama'] ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                            ...
                                            <div class="mb-3">
                                                <label for="username" class="form-label">Username</label>
                                                <input class="form-control" id="username" name="username" value="<?= $a['username'] ?>" placeholder="Masukkan username">
                        ...
                        <!-- Hapus Modal -->
                        <div class="modal fade" id="hapus<?= $a['id'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="editLabel<?= $a['id'] ?>" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
    ...
    <div class="col-lg-4">
        <div class="card p-4 shadow-sm">
            <h4 class="my-2 mb-3 text-center">Tambah Anggota</h4>
            <form class="" method="post" action="<?= Routes::base('anggota/tambah') ?>">
